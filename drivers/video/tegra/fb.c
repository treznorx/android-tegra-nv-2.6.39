...
"%s:application window x offset exceeds display width(%d %d)\n",
dev_name(&win->dc->ndev->dev), win->out_x, xres);
WARN_ONCE(win->out_y >= yres,
"%s:application window y offset exceeds display height(%d %d)\n",
...
win->w.full = win->w.full * new_w / win->out_w;
win->out_w = new_w;
}
if (((win->out_y + win->out_h) > yres) && (win->out_y < yres)) {
long new_h = yres - win->out_y;
win->h.full = win->h.full * new_h / win->out_h;
win->out_h = new_h;
}
...
tegra_dc_incr_syncpt_min(tegra_fb->win->dc, 0, data->syncpt_max);
...
syncpt_max = tegra_dc_incr_syncpt_max(tegra_fb->win->dc, 0);
data->syncpt_max = syncpt_max;

queue_work(tegra_fb->flip_wq, &data->work);

args->post_syncpt_val = syncpt_max;
args->post_syncpt_id = tegra_dc_get_syncpt_id(tegra_fb->win->dc, 0);
